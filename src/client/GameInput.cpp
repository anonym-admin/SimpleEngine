...
void UGameInput::UpdateNdc()
{
	...
	AkF32 fX = (AkF32)_iReleasedMousePosX / (AkF32)uScreenWidth * 2.0f - 1.0f;
	AkF32 fY = (AkF32)_iReleasedMousePosY / (AkF32)uScreenHeight * -2.0f + 1.0f;

	_fReleasedNdcX = fX;
	_fReleasedNdcY = fY;

	fX = (AkF32)_tMousePos.x / (AkF32)uScreenWidth * 2.0f - 1.0f;
	fY = (AkF32)_tMousePos.y / (AkF32)uScreenHeight * -2.0f + 1.0f;

	_fNdcX = Clamp(fX, -1.0f, 1.0f);
	_fNdcY = Clamp(fY, -1.0f, 1.0f);
}
