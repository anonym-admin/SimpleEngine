...
AkU32 FLandScapeObject::sm_uInitRefCount;
ID3D12RootSignature* FLandScapeObject::sm_pRootSignature;
ID3D12PipelineState* FLandScapeObject::sm_pBasicSolidPSO;
ID3D12PipelineState* FLandScapeObject::sm_pBasicWirePSO;
ID3D12PipelineState* FLandScapeObject::sm_pNormalPSO;
ID3D12PipelineState* FLandScapeObject::sm_pDepthOnlyPSO;

FLandScapeObject::FLandScapeObject()
{
}
...
AkBool FLandScapeObject::Initialize(FRenderer* pRenderer)
{
...
void FLandScapeObject::Draw(AkU32 uThreadIndex, ID3D12GraphicsCommandList* pCmdList, const Matrix* pWorldMat)
{
...
void FLandScapeObject::DrawNormal(AkU32 uThreadIndex, ID3D12GraphicsCommandList* pCmdList, const Matrix* pWorldMat)
{
	ID3D12Device* pDevice = _pRenderer->GetDevice();
	FDescriptorPool* pDescriptorPool = _pRenderer->GetDescriptorPool(uThreadIndex);
	ID3D12DescriptorHeap* pDescriptorHeap = pDescriptorPool->GetDescriptorHeap();
...
void FLandScapeObject::DrawShadow(ID3D12GraphicsCommandList* pCmdList, const Matrix* pWorldMat)
{
	ID3D12Device* pDevice = _pRenderer->GetDevice();
...
void* FLandScapeObject::CreateDynamicMeshBuffers(MeshData_t* pMeshData, AkU32 uMeshDataNum)
{
	if (uMeshDataNum != 1)
	{
		__debugbreak();
...
AkBool FLandScapeObject::UpdateMaterialBuffers(const Vector3* pAlbedoFactor, AkF32 fMetallicFactor, AkF32 fRoughnessFactor, const Vector3* pEmisiionFactor)
{
	for (AkU32 i = 0; i < _uMeshNum; i++)
	{
...
HRESULT __stdcall FLandScapeObject::QueryInterface(REFIID riid, void** ppvObject)
{
	return E_NOTIMPL;
}

ULONG __stdcall FLandScapeObject::AddRef(void)
{
	AkU32 uRefCount = ++_uRefCount;
	return uRefCount;
}

ULONG __stdcall FLandScapeObject::Release(void)
{
	AkU32 uRefCount = --_uRefCount;
	if (!uRefCount)
	{
...
DynamicVertexHandle_t* FLandScapeObject::CreateVertexAndIndexBuffer(MeshData_t* pMeshData, AkU32 uMeshDataIndex)
{
...
AkBool FLandScapeObject::CreateCommonResources()
{
	if (sm_uInitRefCount)
	{
		sm_uInitRefCount++;
...
AkBool FLandScapeObject::CreateRootSignature()
{
	ID3D12Device* pDevice = _pRenderer->GetDevice();
...
AkBool FLandScapeObject::CreatePipelineState()
{
	ID3D12Device* pDevice = _pRenderer->GetDevice();
	...
	if (FAILED(D3DCompileFromFile(L"../../shader/LandScapeEditorShader.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PSMain", "ps_5_0", uCompileFlags, 0, &pLandScapeEditorPS, &pErrorBlob)))
	{
		if (pErrorBlob != nullptr)
		{
			OutputDebugStringA((char*)pErrorBlob->GetBufferPointer());
...
void FLandScapeObject::DestroyCommonResources()
{
	if (!sm_uInitRefCount)
	{
		return;
...
void FLandScapeObject::DestroyRootSignature()
{
...
void FLandScapeObject::DestroyPipelineState()
{
	if (sm_pDepthOnlyPSO)
	{
